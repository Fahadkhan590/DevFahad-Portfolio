...
.block-container {padding-top: 1.2rem !important; padding-bottom: 3rem !important; max-width: 1150px !important;}
...
/* Style the actual st.image() portrait — round it and give it a glowing ring */
...
.skill-card p {
    color: #94a3b8; font-size: 0.88rem; line-height: 1.55; margin: 0;
}
.skill-tag {
...
.project-url:hover { color: #34d399 !important; text-decoration: underline !important; }
...
.project-block p {
    color: #cbd5e1; font-size: 0.92rem; line-height: 1.65; margin: 0;
}
...
.contact-card p {
    color: #94a3b8; font-size: 1rem; margin: 0 0 24px 0;
}
.contact-buttons {
...
@media (max-width: 640px) {
    ...
    .hero-wrap { padding: 32px 20px 32px; text-align: center; }
    ...
    .btn, .whatsapp-btn { padding: 11px 18px; font-size: 0.88rem; width: 100%; justify-content: center; }
...
col_photo, col_text = st.columns([1, 1.6], gap="large", vertical_alignment="center")

with col_photo:
    # Directly load and display the profile photo at 300px width
    IMAGE_PATH = Path(__file__).parent / "image_3.webp"
    if IMAGE_PATH.exists():
        st.image(str(IMAGE_PATH), width=300)
    else:
        # Fallback if the image file is missing at runtime
        st.warning("⚠️ `image_3.webp` not found in the app directory. Please add it next to `app.py`.")
